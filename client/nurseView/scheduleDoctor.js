...
function CalendarApp(date) {
    ...
    this.schedule = null
    this.scheduled_appointments = null
  ...
  CalendarApp.prototype.showView = function(date){
    ...
    for ( var z = 1; z <= lastDayOfM; z++ ) {
      ...
      // check if has workingDay to show
      if ( this.aptWorkDays.indexOf(_date.toString()) !== -1 ) {
  ...
  CalendarApp.prototype.openDayWindow = function(date){
    ...
    for (let i=0; i<this.workingHoursArr.length;i++){
        ...
        if (time.getDate() != day.getDate()){
            ...
            scheduleStr+= (time.getHours()<10  ?'0':'')
            scheduleStr+=time.getHours()+":"
            scheduleStr+=(time.getMinutes()<10 ?'0':'')
            scheduleStr+=time.getMinutes()
        }
        if(i%2==0){
            scheduleStr+=" - "
        }
  ...
  CalendarApp.prototype.addEvent = async function() {
    ...
    //tutaj zrobic sprawdzenie czy cleandates zawiera sie w tamtych datach
    if(!IsVisitInsideSchedule(cleanDates, this.workingHoursArr)){
        alert("Sorry, this visit is not during doctor's schedule")
        return
    }
    let appointment = {
        "date" : cleanDates[0],
  ...
  CalendarApp.prototype.addEventFromDB = function() {
