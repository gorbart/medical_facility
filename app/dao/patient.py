...
async def get_medicines(session: AsyncSession, patient_id: str):
    stmt = select(MedicinesTaken, Medicine).join(Medicine, MedicinesTaken.id == Medicine.medicines_taken_id).filter(
        MedicinesTaken.patient_id == patient_id)
...
async def add_medicine_to_patient(session: AsyncSession, patient_id: str, medicine_data: MedicinesTakenInCreate):
    ...
    medicines_taken = await add_entity(session,
                                       MedicinesTaken(**{"date": medicine_data.date, "patient_id": patient.id}))
