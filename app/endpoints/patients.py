...
async def get_one_patient(patient_id: str, session=Depends(get_session)) -> JSONResponse:
    ...
    if patient is not None:
        ...
        res_patient = PatientInResponse(disease_history=[dict(disease)['Disease'].as_dict() for disease in diseases],
                                        medicine_taken=[medicine_taken.to_dict() for medicine_taken in medicines_taken],
                                        **patient.dict())
...
async def get_patient_list(session=Depends(get_session)) -> JSONResponse:
    ...
    for patient in patients:
        ...
        res_patients.append(
            PatientInResponse(disease_history=[dict(disease)['Disease'].as_dict() for disease in diseases],
                              medicine_taken=[medicine_taken.to_dict() for medicine_taken in medicines_taken],
                              **patient.dict()))
...
async def update_patient_data(patient_id: str, received_patient_data: UpdatePatient,
...
async def add_medicine(patient_id: str, medicine_data: MedicinesTakenInCreate,
                       session=Depends(get_session)) -> JSONResponse:
    patient = await get_patient(session, patient_id)
...
async def delete_patient_data(patient_id: str, session=Depends(get_session)) -> Response:
    ...
    if await delete_patient(session, patient_id):
